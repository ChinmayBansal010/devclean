...
std::vector<CategoryStat> sortCategories(const std::map<std::string, uint64_t>& categoryBytes,
...
{
    if (args.activeOnly)
        results.erase(std::remove_if(results.begin(), results.end(), [](const ScanResult& result) { return !result.active; }), results.end());
    if (args.minSizeBytes > 0)
        results.erase(std::remove_if(results.begin(), results.end(), [&](const ScanResult& result) { return !result.found || result.bytes < args.minSizeBytes; }), results.end());
    if (args.maxSizeBytes > 0)
        results.erase(std::remove_if(results.begin(), results.end(), [&](const ScanResult& result) { return !result.found || result.bytes > args.maxSizeBytes; }), results.end());
}
}
...
int StatsCommand::execute(const ParsedArgs& args)
{
    ...
    for (const auto& result : results)
    {
        ...
        if (result.active) ++activeCount;
        else if (result.found) ++inactiveCount;
        if (result.found)
        {
            ++foundCount;
    ...
    if (args.json)
    {
        ...
        payload["inactive_count"] = inactiveCount;
        ...
        payload["health"] = {{"score", insights.health.score}, {"label", insights.health.label}, {"factors", insights.health.factors}};
        payload["trend"] = {{"latest_growth_bytes", insights.latestGrowthBytes}, {"latest_growth_percent", insights.latestGrowthPercent}, {"average_growth_bytes", insights.averageGrowthBytes}};
        payload["recommendations"] = nlohmann::json::array();
        for (const auto& recommendation : insights.recommendations)
            payload["recommendations"].push_back({{"name", recommendation.name}, {"reason", recommendation.reason}, {"action", recommendation.action}, {"bytes", recommendation.bytes}, {"growth_bytes", recommendation.growthBytes}, {"priority", recommendation.priority}, {"safe", recommendation.safe}});
        payload["largest_caches"] = nlohmann::json::array();
        for (std::size_t i = 0; i < std::min<std::size_t>(largestCaches.size(), 5); ++i)
            payload["largest_caches"].push_back({{"name", largestCaches[i].first}, {"bytes", largestCaches[i].second}});
        payload["largest_categories"] = nlohmann::json::array();
        for (const auto& [category, stat] : categories)
            payload["largest_categories"].push_back({{"name", category}, {"bytes", stat.first}, {"count", stat.second}});
    ...
    std::cout << "Statistics\n----------\n";
    std::cout << "Filters: ";
    if (!args.activeOnly && args.minSizeBytes == 0 && args.maxSizeBytes == 0) std::cout << "none\n";
    else
    {
        bool separator = false;
        if (args.activeOnly) { std::cout << "active-only"; separator = true; }
        if (args.minSizeBytes > 0) { if (separator) std::cout << ", "; std::cout << "min=" << Formatter::formatBytes(args.minSizeBytes); separator = true; }
        if (args.maxSizeBytes > 0) { if (separator) std::cout << ", "; std::cout << "max=" << Formatter::formatBytes(args.maxSizeBytes); }
        std::cout << '\n';
    }
    std::cout << "Caches scanned: " << results.size() << '\n';
    std::cout << "Caches found: " << foundCount << '\n';
    std::cout << "Active tools: " << activeCount << '\n';
    std::cout << "Inactive tools: " << inactiveCount << '\n';
    ...
    std::cout << "Latest growth: " << Formatter::formatSignedBytes(insights.latestGrowthBytes) << " (" << (insights.latestGrowthPercent >= 0.0 ? "+" : "") << insights.latestGrowthPercent << "%)\n";
    std::cout << "Average growth: " << Formatter::formatSignedBytes(insights.averageGrowthBytes) << " per snapshot\n";
    if (!insights.recommendations.empty()) std::cout << "Top recommendation: " << insights.recommendations.front().name << " - " << insights.recommendations.front().action << '\n';
    std::cout << "\nLargest caches:\n";
    for (std::size_t i = 0; i < std::min<std::size_t>(largestCaches.size(), 5); ++i)
