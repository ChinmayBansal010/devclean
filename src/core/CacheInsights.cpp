...
{
    if (totalBytes >= 1ULL * TB)
        return 40;
    if (totalBytes >= 512ULL * GB)
        return 35;
    if (totalBytes >= 256ULL * GB)
        return 30;
    if (totalBytes >= 128ULL * GB)
        return 26;
    if (totalBytes >= 64ULL * GB)
        return 22;
    if (totalBytes >= 32ULL * GB)
        return 18;
    if (totalBytes >= 16ULL * GB)
        return 14;
    if (totalBytes >= 8ULL * GB)
        return 10;
    if (totalBytes >= 4ULL * GB)
        return 6;
    if (totalBytes >= 2ULL * GB)
        return 3;
    return 0;
}
...
int penaltyForGrowth(int64_t growthBytes)
{
    if (growthBytes <= 0)
        return 0;
    if (growthBytes >= static_cast<int64_t>(8ULL * GB))
        return 20;
    if (growthBytes >= static_cast<int64_t>(4ULL * GB))
        return 16;
    if (growthBytes >= static_cast<int64_t>(2ULL * GB))
        return 12;
    if (growthBytes >= static_cast<int64_t>(1ULL * GB))
        return 8;
    if (growthBytes >= static_cast<int64_t>(512ULL * MB))
        return 5;
    if (growthBytes >= static_cast<int64_t>(256ULL * MB))
        return 3;
    return 1;
}
                                 ...
                                 const std::vector<ScanSnapshot>& history)
{
    ...
    for (const auto& result : results)
    {
        ...
        const bool safe = !result.active && result.warnings.empty() && !Filesystem::isProtectedPath(result.location);

        if (safe && result.bytes >= 1ULL * GB)
        {
            recommendations.push_back(makeRecommendation(
                result,
                100,
                "inactive cache without warnings above 1 GB",
        ...
        else if (result.growthBytes >= static_cast<int64_t>(512ULL * MB))
        {
            recommendations.push_back(makeRecommendation(
                result,
                95,
                "cache grew by " + formatSignedBytes(result.growthBytes) + " since the previous scan",
                "Inspect this cache before it grows further",
                safe));
        }
        else if (currentAgeDays >= 90 && result.bytes >= 250ULL * MB)
        {
            recommendations.push_back(makeRecommendation(
                result,
                85,
                "cache is old and still consuming meaningful disk space",
                "Review for cleanup",
                safe));
        }
        else if (!result.warnings.empty())
        {
            recommendations.push_back(makeRecommendation(
                result,
                70,
                "tool warnings suggest the cache may still be active",
                "Use safe cleanup or wait for the tool to finish",
                false));
        }
        else if (result.active && result.bytes >= 500ULL * MB)
        {
            recommendations.push_back(makeRecommendation(
                result,
                60,
                "large cache is associated with an active tool",
                "Defer cleanup until the tool is idle",
                false));
        }
        else if (result.bytes >= 500ULL * MB)
        {
            recommendations.push_back(makeRecommendation(
                result,
                55,
                "cache is large enough to be worth a cleanup review",
        ...
        else if (currentAgeDays >= 30 && result.bytes >= 100ULL * MB)
        {
            recommendations.push_back(makeRecommendation(
                result,
                50,
                "cache is old enough to justify a review",
                "Consider clearing it if the tool is no longer needed",
                safe));
        }
    }
    ...
    for (const auto& result : results)
    {
        if (!result.found)
            continue;
        agePenalty += penaltyForAge(ageDays(result));
    }
    ...
    if (insights.foundCount == 0)
        score = 100;
    else if (insights.latestGrowthBytes < 0)
        score = std::min<int64_t>(100, score + 4);
    ...
    {
        if (totalPenalty > 0)
            insights.health.factors.push_back("disk usage pressure: -" + std::to_string(totalPenalty));
        if (growthPenalty > 0)
            insights.health.factors.push_back("growth pressure: -" + std::to_string(growthPenalty));
        if (warningPenalty > 0)
            insights.health.factors.push_back("warnings: -" + std::to_string(warningPenalty));
        if (activePenalty > 0)
            insights.health.factors.push_back("active caches: -" + std::to_string(activePenalty));
        if (agePenalty > 0)
            insights.health.factors.push_back("age pressure: -" + std::to_string(agePenalty));
        if (insights.latestGrowthBytes < 0)
            insights.health.factors.push_back("recent growth is negative, which improves the score");
    }
