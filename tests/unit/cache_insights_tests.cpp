...
int main()
{
    ...
    ScanResult cargo;
    cargo.name = "cargo";
    cargo.bytes = 3ULL * 1024 * 1024 * 1024;
    cargo.files = 10;
    cargo.directories = 2;
    cargo.found = true;
    cargo.active = false;
    ...
    cargo.growthBytes = 512LL * 1024 * 1024;
    ScanResult npm;
    npm.name = "npm";
    npm.bytes = 250ULL * 1024 * 1024;
    npm.files = 4;
    npm.directories = 1;
    npm.found = true;
    npm.active = true;
    ...
    latest.totalFiles = 14;
    latest.totalDirectories = 3;
    ScanSnapshot previous;
    previous.timestamp = now - std::chrono::hours(24);
    previous.totalBytes = 2ULL * 1024 * 1024 * 1024;
    ...
    assert(cacheHealthLabel(74) == "Watch");
    assert(cacheHealthLabel(59) == "At risk");
    return 0;
}
