...
int main()
{
    ...
    assert(insights.recommendations.front().safe); assert(!insights.recommendations.back().safe); assert(insights.largestCaches.front().first == "cargo");
    assert(insights.cacheGrowth.front().first == "cargo");
    return 0;
}
